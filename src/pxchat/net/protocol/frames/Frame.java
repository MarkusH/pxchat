...
/**
 * This class is the base class for all data transferred over
 * the network. It implements the {@link Serializable}
 * interface so that it can be written to the output stream
 * of the socket.
 * 
 * Each subclass of <code>Frame</code> must have an <code>id</code>
 * that is unique among the set of all frames.
 * 
...
public abstract class Frame implements Serializable {
	...
	/**
	 * Constructs a new <code>Frame</code>. The id is
	 * set to <i>ID_NOP</i>.
	 */
	public Frame() { 
	}
